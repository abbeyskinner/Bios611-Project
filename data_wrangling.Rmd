...
model1 <- lm(data = contestant_data, normalized_finish ~ age + gender + poc +
                 win_percentage + advantage + success + job_category +
               confessionals_per_episode + mb_ei)
summary(model1)
plot(model1)
...
model <- lm(data = train_data, finish ~ age + gender + poc +
                 win_percentage + advantage + success + job_category +
               confessionals_per_episode + mb_ei)
summary(model)
plot(model)
...
pred_vals <- predict(model, test_data) %>% as.tibble() %>%
  mutate(
    ...
    observed = test_data$finish,
    residual = observed - predicted
  ) %>%
  select(-1) %>%
  filter(residual > -4 & residual < 4)



%>%
  ggplot(aes(x=observed, y=predicted)) + geom_point()

model$fitted.values %>% as.tibble()
pred_vals
```
...
regression_data <- contestant_data %>%
  select(normalized_finish, finish, age, gender, race, poc, region, job_category, 
         win_percentage, num_advantages, advantage_success_rate, num_appearance,
         confessionals_per_episode, mb_ei, mb_sn, mb_tf, mb_jp, personality_type)
...
finish_model <- lm(normalized_finish ~ age + gender + poc + 
                      win_percentage + num_advantages + advantage_success_rate + num_appearance +
                   confessionals_per_episode + mb_ei, 
...
```{r}
ggplot(contestant_data %>% drop_na(personality_type), aes(x = fct_reorder(personality_type, normalized_finish, .fun = median), 
...
contestant_data %>%
  group_by(finish) %>%
  summarise(
    mean_confessionals = sum(confessionals_per_episode)/n()
  ) %>%
  ggplot(aes(x = finish, y = mean_confessionals, fill = finish)) +
  geom_bar(stat = "identity", show.legend = FALSE) +  # Use stat = "identity" for pre-calculated values
  geom_hline(yintercept = mean(contestant_data$confessionals_per_episode), color = "red", size = 1) +  # Add  line at overall mean
 # geom_text(aes(label = round(mean_finish, 2)), vjust = -0.3) + # Add labels with rounded mean values
  labs(
    x = "Finish",
    y = "Mean Confessionals per Episode",
    title = "Mean Confessionals per Episode by Finish"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels for readability
  theme_minimal() # Clean theme
...
contestant_data %>%
  group_by(normalized_finish) %>%
  summarise(
    mean_confessionals = sum(confessionals_per_episode)/n()
  ) %>%
  ggplot(aes(x=as.factor(normalized_finish), y= mean_confessionals)) +
...
contestant_data %>%
  ...
  group_by(finish) %>%
  summarise(
    average_success_rate = sum(advantage_success_rate)/n()
  ) %>%
  ggplot(aes(x = finish, y = average_success_rate, fill = finish)) +
  geom_bar(stat = "identity", show.legend = FALSE) +  # Use stat = "identity" for pre-calculated values
 # geom_text(aes(label = round(mean_finish, 2)), vjust = -0.3) + # Add labels with rounded mean values
  labs(
    x = "Finish",
    y = "Average Success Rate",
    title = "Average Advantage Success Rate by Finish"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels for readability
  theme_minimal() # Clean theme
```
...
rf_data <- contestant_data %>%
  ...
  mutate(
    ...
    mb_tf = as.factor(mb_tf),
    mb_jp = as.factor(mb_jp)  ) %>% 
  drop_na(personality_type) %>%
  select(-c(normalized_finish, mb_ei, mb_sn, mb_tf, mb_jp))

set.seed(220)
train_data <- rf_data %>% sample_frac(0.7)
test_data <- setdiff(rf_data, train_data)

# Random forest model
rf_model <- randomForest(train_data$finish ~ ., data = train_data[,-1], ntree = 500, mtry = 2, importance = TRUE)
...
rf_predictions <- predict(rf_model, newdata = test_data) %>% as.tibble()

predict(rf_model, newdata = test_data) %>% as.tibble() %>%
  mutate(observed = test_data$finish,
         expected = round(value),
         residual = observed - expected) %>%
  filter(residual < 4,
         residual > -4)

mse <- mean((test_data$finish - rf_predictions)^2)
print(paste("Mean Squared Error:", mse))

ss_total <- sum((test_data$finish - mean(test_data$finish))^2)
ss_residual <- sum((test_data$finish - rf_predictions)^2)
r_squared <- 1 - (ss_residual / ss_total)

plot(test_data$finish, rf_predictions,
     xlab = "Actual Values",
     ylab = "Predicted Values",
     main = "Actual vs. Predicted")
abline(0, 1, col = "blue", lwd = 2) # Diagonal line for perfect prediction
