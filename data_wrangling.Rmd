...
contestant_data3 %>%
  ...
  ggplot(aes(x = gender, y = mean_finish, fill=gender)) +
  geom_bar(stat = "identity")+  # Use stat = "identity" for pre-calculated values
  geom_hline(yintercept = mean(contestant_data3$normalized_finish), color = "red", size = 1) +  # Add  line at overall mean
 # geom_text(aes(label = round(mean_finish, 2)), vjust = -0.3) + # Add labels with rounded mean values
  labs(
    x = "Gender",
    y = "Mean Finish",
...
ggplot(aes(x = lgbt, y = mean_finish, fill = race)) +
  ...
  geom_hline(yintercept = mean(contestant_data3$finish), color = "red", size = 1) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  # Rotate x-axis labels for readability
  theme_minimal() +  # Clean theme
...
```{r}
model_data <- contestant_data3 %>%
  select(age, gender, poc, jewish, muslim, lgbt, num_appearance, 
         votes_against, win_percentange, num_advantages, advantage_success_rate) %>%
...
pca_data <- model_data  %>% as.matrix()

pca_result <- prcomp(model_data %>% as.matrix(), scale. = TRUE)
pcs <- pca_result$x %>% 
  as_tibble() %>% 
  select(PC1, PC2, PC3, PC4, PC5, PC6) %>%
  mutate(
    finish = contestant_data3$finish
  )

data_with_pcs <- contestant_data3 %>%
  mutate(
    PC1 = pcs$PC1,
    PC2 = pcs$PC2,
    PC3 = pcs$PC3
...
## poc, gender (pc2 - and pc1??), advantage, success, num_season pc1
# the 4 points in the bottom right corner are all muslim contestants 
```

```{r}
kmeans_results <- kmeans(model_data, 3)
...
```{r}
model1 <- lm(data = contestant_data3, finish ~ age + gender + poc + lgbt +
               num_appearance + win_percentange + advantage + success + job_category)
summary(model1)
plot(model1)

set.seed(220)
train_data <- contestant_data3 %>% sample_frac(0.7)
test_data <- setdiff(contestant_data3, train_data)

model <- lm(data = train_data, finish ~ age + gender + poc + lgbt +
               num_appearance + win_percentange + advantage + success + job_category)
summary(model)
plot(model)
...
plot(pred_vals, test_data$finish)
...
regression_data <- contestant_data3 %>%
  select(normalized_finish, age, gender, race, region, job_category, 
         win_percentange, num_advantages, advantage_success_rate, num_appearance)
...
```{r}
library(forcats)
ggplot(contestant_data3, aes(x = fct_reorder(job_category, normalized_finish, .fun = median), 
    ...
    theme_minimal() +
    geom_hline(yintercept = mean(contestant_data3$normalized_finish), 
...
contestant_data3 %>%
  ...
  geom_hline(yintercept = mean(contestant_data3$normalized_finish, na.rm = TRUE), 
...
contestant_data3 %>%
  group_by(job_category) %>%
  summarise(mean_win_percentage = mean(win_percentange, na.rm = TRUE)) %>%
...
```{r}
contestant_data3 %>% filter(advantage == 1) %>%
  group_by(job_category) %>%
  summarise(
    n = n(),
 ...
 ggplot(aes(x = reorder(job_category, avg_success_rate), y = avg_success_rate)) +
  ...
  coord_flip() +
  geom_hline(yintercept = mean((contestant_data3 %>% filter(advantage == 1))$advantage_success_rate), 
             color = "red", size = 1) +  # Add  line at overall mean
